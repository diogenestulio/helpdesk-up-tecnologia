...
def init_db():
    ...
    c.execute("INSERT OR IGNORE INTO usuarios VALUES (?,?,?,?,?)",
              ("admin", "admin123", "00.000.000/0001-00", "Administrador", "admin"))
    conn.commit()
    conn.close()
...
def autenticar(u, p):
    ...
    conn.close()
    if res:
        return {"user": u, "tipo": res[0], "cnpj": res[1], "nome": res[2]}
    return None
...
if not st.session_state["auth"]:
    ...
    with colB:
        st.markdown(f"""
        <div style="padding:16px; backdrop-filter: blur(8px); background:{GLASS}; border-radius:16px; border:1px solid #eaeaea;">
            <h4 style="margin-top:0; color:{PRIMARY};">Transparência e controle</h4>
            <ul>
                <li>Acompanhe seus chamados em tempo real</li>
                <li>Veja status, etapas, datas e valores</li>
                <li>Relatórios financeiros por CNPJ e mês</li>
            </ul>
        </div>
        """, unsafe_allow_html=True)
    st.stop()
...
if menu == "Abrir Chamado":
    ...
    with st.form("novo_ch"):
        ...
        enviar = st.form_submit_button("Enviar para Up Tecnologia", type="primary")
    ...
    <div style="margin-top:16px; padding:12px; background:{BG_LIGHT}; border-radius:12px; border:1px solid #eaeaea;">
        <strong>Transparência:</strong> Você verá seu chamado no Kanban com status e etapa atualizados, incluindo data/hora e quem solicitou.
    </div>
    """, unsafe_allow_html=True)

# -----------------------------
# KANBAN DE CHAMADOS (ADMIN/CLIENTE)
# -----------------------------
        ...
        with col:
            ...
            for _, r in subset.iterrows():
                ...
                <div style="margin-bottom:12px; padding:12px; backdrop-filter: blur(6px); background:{GLASS}; border-radius:12px; border:1px solid #eaeaea;">
                    ...
                    <small>Etapa: {r['etapa']} • CNPJ: {r['cnpj']} • Autor: {r['autor']} • Abertura: {r['data_abertura']} • Valor: R$ {r['valor']:.2f}</small>
                </div>
                """, unsafe_allow_html=True)

    # Atualização rápida (admin)
    if perfil == "admin":
        st.divider()
...
# -----------------------------
# CADASTRO EMPRESA/RESPONSÁVEL (ADMIN)
# -----------------------------
        ...
        with colS2:
            if st.button("💾 Salvar usuários"):
                ...
                for _, r in edited_usr.iterrows():
                    conn.execute("""INSERT OR REPLACE INTO usuarios (username, senha, cnpj, nome_completo, tipo)
                                    VALUES (?,?,?,?,?)""", (r["username"], "SenhaInicial123!", r["cnpj"], r["nome_completo"], r["tipo"]))
                conn.commit()
                conn.close()
...
# -----------------------------
    ...
    if perfil != "admin":
        ...
        # Filtra chamados do CNPJ e mês/ano
        conn = get_conn()
        df_fin = pd.read_sql_query("SELECT * FROM chamados WHERE cnpj=?", conn, params=(cnpj_sel,))
        conn.close()

        # Extrai mês/ano da data_abertura (formato dd/mm/YYYY HH:MM)
        def match_mes_ano(s):
            try:
        ...
        if not df_fin.empty:
            ...
            st.dataframe(df_fin[["id", "data_abertura", "autor", "problema", "status", "etapa", "valor"]], use_container_width=True)
            fig_val = px.bar(df_fin, x="data_abertura", y="valor", color="status", title="Valores por chamado (linha do tempo)",
